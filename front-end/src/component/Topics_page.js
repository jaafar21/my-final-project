    ...
    if (isEnd) {
      ...
      return (
        <div className="topics_container">
          ...
          {myTopic !== undefined ? (
            <div>
              <h1 className="topics_title">{myTopic.title}</h1>
              <div className="top">
                <img
                  className="topics_image"
                  src={`http://localhost:8001/${myTopic.image}`}
                />
              </div>
              <p className="topics_decription"> {myTopic.discription} </p>
            </div>
          ) : (
            "ss"
          )}
          {currentTopic < topics.length - 1 ? (
            ...
            <button className="topics_button" onClick={this.finishHandler}>
              exam
            </button>
          )}
        </div>
